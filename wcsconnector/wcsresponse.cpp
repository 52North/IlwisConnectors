...
WcsResponse::WcsResponse() // TODO: rename to wfsconnection?!
{
    initialize();
    _finished = false;
}

WcsResponse::WcsResponse(QIODevice *device): _iodevice(device)
{
    initialize();
    _finished = true;
}

void WcsResponse::initialize()
{
...
bool WcsResponse::hasFinished() const
{
    return _finished;
}

bool WcsResponse::isException() const
{
    if (!hasFinished()) {
        return false;
    }
...
QString WcsResponse::parseException() const
{
...
QNetworkReply *WcsResponse::performRequest(QNetworkRequest &request, bool async)
{
    if (async) {
...
void WcsResponse::cancelRequest()
{
    ERROR0("The request will stop due to timeout. Check your internet connection.");
    _reply->abort();
}

QIODevice *WcsResponse::device() const
{
    return _iodevice;
}

void WcsResponse::setDevice(QIODevice *device)
{
    _iodevice = device;
    _finished = true;
}

void WcsResponse::readResponse(QNetworkReply *reply)
{
